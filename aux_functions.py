...
color_palette = [
    '#ffa600', '#ff7c43', '#f95d6a', '#d45087', '#a05195', '#665191',
    '#2f4b7c', '#003f5c'
]
...
# Databases
df_target = pd.read_excel(r'target_prices.xlsx',
                          header=1,
                          usecols=[
                              'ticker', 'ticker_int', 'type', 'target_price',
                              'currency', 'superman', 'batman', 'princesa'
                          ])
...
def calculate_stock_score(ticker, target_price, currency):
    ...
    try:
        ...
        score = (1 + margin)**2 * (1 - 1.5 * var_1) * (1 - 1.3 * var_2) * (
            1 - 1 * var_3)
        if (margin < 0) or (score < 0):
            score = 0
    except:
        margin = None
        score = None
...
def invest_cleaning(df_raw):
    ...
    df_raw['ticker'] = np.where(df_raw['type'].isin(['int', 'nac', 'fii']),
                                df_raw['Produto'].apply(get_ticker), None)
    df_raw = df_raw.rename(
        columns={
            'Valor aplicado': 'value_applied',
    ...
    df = df_raw[[
        'ticker', 'type', 'value_applied', 'gross_balance', 'profit',
        'percentage'
    ]]
...
def get_score(df):
    ticker = df['ticker']
    try:
        ticker_int = df_target.loc[df_target['ticker'] == ticker,
                                   'ticker_int'].values[0]
    except:
        ticker_int = None
...
def get_recomendations_from_type(df, type_option):
    ...
    df_type = df.loc[df['type'] == product_type, :].drop_duplicates(
        subset=['ticker'])
    ...
    df_plot = df_plot.sort_values(by='score',
...
def get_commend_values(df_clean, df_portfolio, cash, investor, n_commends):
    ...
    # Minimization
    def fun(x):
        return calculate_mape(df_inv['ideal_balance'].values,
                              df_inv['gross_balance'].values, x, n_commends)
    ...
    for i, col in enumerate(cols):
        ...
        with col:
            st.metric(
                df_commend.loc[i, 'type_name'],
                'R${:,.0f}'.format(df_commend.loc[i, 'commend_value']),
                '{:.2f}%'.format(df_commend.loc[i, 'variation_percentage']))
...
def get_stocks_recommendation_from_type(df_sell, investor, commend_value,
    ...
    df['ideal_balance'] = (
        df_sell.loc[df_sell['type'] == invest_type, 'gross_balance'].sum() +
        commend_value) / len(df)
    df = df.sort_values('weights', ascending=False).reset_index(drop=True)

    # Minimization
    def fun(x):
        return calculate_mape(df['ideal_balance'].values,
                              df['gross_balance'].values, x, n_stocks)
    ...
    df_stocks = df[(df['commend_value'] > 0)
                   & (df['score_scaled'] > 0)].reset_index(drop=True)
    ...
    for i, col in enumerate(cols):
        with col:
            try:
                ...
                var = '-'
            st.metric(df_stocks.loc[i, 'ticker'],
                      'R${:,.0f}'.format(df_stocks.loc[i, 'commend_value']),
                      var,
                      delta_color='inverse')
...
def show_stock_commends(df_clean, df_commend, investor):
    ...
    sell_values = df_sell.loc[(df_sell['in_portfolio'] == 0) &
                              (df_sell['gross_balance'] > 0),
                              'gross_balance'].values
    ...
    if len(sell_stocks) > 0:
        ...
        for i, col in enumerate(cols):
            with col:
                st.metric(sell_stocks[i], 'R${:,.0f}'.format(sell_values[i]),
                          '{:.2f}%'.format(sell_profits[i]))

    types_to_commend = df_commend.loc[df_commend['type'].isin(stock_types),
                                      'type'].values
    for invest_type in types_to_commend:
        ...
        commend_value = df_commend.loc[df_commend['type'] == invest_type,
                                       'commend_value'].values[0]
        max_n_stocks = int(
            (df_target[(df_target[investor] == 1)
                       & (df_target['type'] == invest_type)] != 0).sum(
                           axis=0).values[0])
        n_stocks = st.slider('Deseja recomendação de quantos ativos?'.format(),
                             min_value=1,
                             max_value=max_n_stocks,
                             value=3,
                             step=1)
