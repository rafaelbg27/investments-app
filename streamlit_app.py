...
if file:
    ...
    if best_option:
        portfolio_option = st.selectbox(
            ...
            ['-'] + (list(df_portfolio['investor_type'].str.capitalize())))
        cash = st.number_input('Valor do aporte: R$')

        if cash and portfolio_option != '-':

            st.markdown('***')
            max_n_cats = int((df_portfolio[df_portfolio['investor_type'] ==
                                           portfolio_option.lower()] != 0).sum(
                                               axis=1).values[0] - 1)
            n_commends = st.slider(
                'Em quantas categorias de investimento você quer distribuir seu aporte?',
                min_value=1,
                max_value=max_n_cats,
                value=2,
                step=1)

            df_commend = aux_functions.get_commend_values(
                df_clean, df_portfolio, cash, portfolio_option.lower(),
                n_commends)

            aux_functions.show_stock_commends(df_clean, df_commend,
                                              portfolio_option.lower())

    if manual_option:
        product_option = st.selectbox(
            'Onde você quer investir?',
            ['Ações nacionais', 'Ações internacionais', 'Fundos Imobiliários'])

        aux_functions.get_recomendations_from_type(df_clean, product_option)
